...
import static com.custom.couponcore.exception.ErrorCode.COUPON_NOT_EXIST;
...
public class CouponIssueService {
    ...
    public Coupon findCoupon(long couponId) {
        return couponJpaRepository.findById(couponId).orElseThrow(() -> {
            throw new CouponIssueException(COUPON_NOT_EXIST, "쿠폰 정책이 존재하지 안습니다. %s".formatted(couponId));
        });
    }
    ...
    private void checkAlreadyIssuance(long couponId, long userId) {
        ...
        if(issue != null) {
            throw new CouponIssueException(DUPLICATED_COUPON_ISSUE,"이미 발급된 쿠폰입니다. user_id : %s, cunpon_id : %s".formatted(userId,couponId));
        }
    }
}
